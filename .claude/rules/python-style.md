...
CI gate: `ruff format --check .` → `ruff check .` → `pyright` → `pytest` (in that order).
...
- `target-version = "py312"` — use modern syntax: `X | Y` unions, `list[str]` not `List[str]`
...
- Do NOT use `from __future__ import annotations` in pydantic model files — breaks runtime type evaluation
