...
2. **Type safety** — public functions must have full type annotations; use `str | None` not `Optional`; run `uv run pyright` and report any new errors; no blanket `# type: ignore` (use `# pyright: ignore[reportSpecificRule]` with comment); no `cast()` to hide real mismatches; no `Any` as a crutch; no `from __future__ import annotations` in pydantic model files
...
8. **Test quality** — new public methods need tests; mocks use `spec=RealClass`; assert outcomes not call order; factory functions for variations; no `setup_method/teardown_method`; shared fixtures from `conftest.py` reused; `@pytest.mark.parametrize` for 3+ identical-structure tests
9. **Style** — 100-char line length; loguru `{}` placeholders; ruff-clean (`uvx ruff check .`); modern syntax: `X | Y` not `Optional[X]`, `list[str]` not `List[str]`; `raise ... from err` in except blocks (B904); no stray `print()` in production code (T20); imports sorted (I001)
