...
from bot.services.supabase_manager import SupabaseManager
...
async def store_webhook_event(
    ...
    try:
        ...
        result = supabase.service_client.table("webhook_events").insert(event_data).execute()
...
async def is_event_processed(supabase: SupabaseManager, provider: str, event_id: str) -> bool:
    """
    ...
    try:
        result = (
            supabase.service_client.table("webhook_events")
            .select("id")
...
async def handle_checkout_session_completed(
    ...
    if not user_id and customer_id:
        ...
        result = (
            supabase.service_client.table("users")
            .select("id")
    ...
    try:
        ...
        if not plan_id and stripe_sub.get("items", {}).get("data"):
            ...
            plans = (
                supabase.service_client.table("subscription_plans")
                .select("id")
        ...
        result = (
            supabase.service_client.table("user_subscriptions").insert(subscription_data).execute()
        )
...
async def handle_invoice_payment_succeeded(supabase: SupabaseManager, data: Dict[str, Any]) -> None:
    ...
    # Find subscription
    result = (
        supabase.service_client.table("user_subscriptions")
        .select("id, user_id")
    ...
    (supabase.service_client.table("payments").insert(payment_data).execute())
...
async def handle_invoice_payment_failed(supabase: SupabaseManager, data: Dict[str, Any]) -> None:
    ...
    (
        supabase.service_client.table("user_subscriptions")
        .update({"status": "past_due", "updated_at": datetime.utcnow().isoformat()})
        .eq("id", subscription["id"])
        .execute()
    )
...
async def handle_paddle_subscription_created(
    ...
    subscription_data = {
        ...
        "provider_subscription_id": subscription_id,
        "provider_customer_id": customer_id,
        "metadata": {"paddle_subscription_data": data},
    }
...
async def handle_paddle_transaction_completed(
    ...
    payment_data = {
        ...
        "metadata": {"paddle_transaction_data": data},
    }
