...
class CogneeClient:
    ...
    async def delete_dataset(self, dataset_name: str) -> None:
        ...
        logger.warning(
            "delete_dataset called for {} but cognee provides no dataset-scoped delete; "
            "skipping to avoid wiping all users.",
            dataset_name,
        )
...
class CogneeMemoryService:
    ...
    async def get_all_memories(
        ...
        user_dataset = self._user_dataset(user_id)
        try:
            results = await self._client.search(query_text="memories", datasets=[user_dataset])
