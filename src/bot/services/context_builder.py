...
class ContextBuilder:
    ...
    def _filter_memories(
        ...
        if self.config.include_categories:
            filtered = [
                m for m in filtered 
                if m.memory_category in self.config.include_categories
            ]
        if self.config.exclude_categories:
            filtered = [
                m for m in filtered 
                if m.memory_category not in self.config.exclude_categories
            ]
        ...
        if self.config.include_types:
            filtered = [
                m for m in filtered 
                if m.memory_type in self.config.include_types
            ]
        if self.config.exclude_types:
            filtered = [
                m for m in filtered 
                if m.memory_type not in self.config.exclude_types
            ]
        
        # Filter by importance score
        filtered = [
            m for m in filtered 
            if m.get_effective_importance() >= self.config.min_importance_score
        ]
    ...
    def build_recent_messages_part(
        ...
        if token_estimate > self.config.max_recent_messages_tokens:
            max_chars = int(
                self.config.max_recent_messages_tokens / self.config.tokens_per_char
            )
    ...
    def assemble(
        ...
        parts: dict[str, ContextPart | None] = {
            ...
            "semantic_memory": self.build_semantic_memory_part(
                semantic_memories or [], query
            ),
        }
