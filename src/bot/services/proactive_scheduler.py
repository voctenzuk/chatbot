...
Architecture:
    ...
    └──────┬───────────┘
           ...
           └── IntervalTrigger(30m)  → _idle_check()
                        │
                        ▼
             Anti-spam → LLM generate → bot.send_message
"""
...
class ProactiveScheduler:
    ...
    async def send_proactive_message(self, user_id: int, prompt_hint: str) -> bool:
        ...
        Args:
            ...
            prompt_hint: Context hint for LLM (e.g., "morning greeting").

        Returns:
            True if message was sent, False if skipped/failed.
        """
        ...
        try:
            ...
            try:
                from bot.services.episode_manager import get_episode_manager
